<template>
  <div class="wrapper" ref="myscroll">
    <!-- 商品列表 start -->
    <ul class="messageList bgeeeeee" v-if="complaintList.length">
      <!-- 商品简介 start -->
      <li v-for="(item, index) in complaintList" :key="index">
        <router-link :to="{name: 'complaintDetail', params: {id: item.id}}" tag="div" class="message_item bgffffff">
          <div class="bgffae43 border_r500"></div>
          <div class="message_title ellipsis">{{item.content}}</div>
...
<style scoped>
@import 'static/css/myscrollComplaint.css';
